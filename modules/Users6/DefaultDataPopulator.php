...
class DefaultDataPopulator extends CRMEntity {
	...
	function create_tables() {
		...
		$this->db->query("insert into vtiger_tab(tabid,name,presence,tabsequence,tablabel,customized,ownedby,isentitytype,parent) values (29,'Users6',0,-1,'Users6',0,1,0,null)");
		...
		$this->db->query("insert into vtiger_blocks values (" . $userblockid . ",29,'LBL_USER_IMAGE_INFORMATION',4,0,0,0,0,0,1,0)"); //Added a New Block User Image Info in Users6 Module
		$useradvanceblock = $this->db->getUniqueID('vtiger_blocks');
		$this->db->query("insert into vtiger_blocks values (" . $useradvanceblock . ",29,'LBL_USER_ADV_OPTIONS',5,0,0,0,0,0,1,0)"); //Added a New Block User Image Info in Users6 Module
		...
		$this->db->query("insert into vtiger_entityname values(29,'Users6','vtiger_users','first_name,last_name','id','id')");
		...
		$this->db->query("insert into vtiger_relatedlists values(" . $this->db->getUniqueID('vtiger_relatedlists') . "," . getTabid("Emails") . ",0,'get_users',2,'Users6',0,'')");
		...
		$this->db->query("insert into vtiger_relatedlists values (" . $this->db->getUniqueID('vtiger_relatedlists') . "," . getTabid("Calendar") . ",0,'get_users',1,'Users6',0,'')");
...
//for Support end notification before a month
	...
	function vtws_addEntityInfo() {
		...
		$entityId = $this->db->getUniqueID("vtiger_ws_entity");
		$this->db->pquery('insert into vtiger_ws_entity(id,name,handler_path,handler_class,ismodule) values (?,?,?,?,?)', array($entityId, 'Users6', $moduleHandler['file'], $moduleHandler['class'], 1));
	...
	function vtws_addReferenceTypeInformation() {
		$referenceMapping = array("50" => array("Accounts"), "51" => array("Accounts"), "57" => array("Contacts"),
			...
			"78" => array("Quotes"), "80" => array("SalesOrder"), "81" => array("Vendors"), "101" => array("Users6"), "52" => array("Users6"),
			"357" => array("Contacts", "Accounts", "Leads", "Users6", "Vendors"), "59" => array("Products"),
			"66" => array("Leads", "Accounts", "Potentials", "HelpDesk", "Campaigns"), "77" => array("Users6"), "68" => array("Contacts", "Accounts"),
		...
		$result = $this->db->pquery("insert into vtiger_ws_entity_referencetype(fieldtypeid,type) values(?,?)", array($fieldTypeId, 'Users6'));
		if (!is_object($result)) {
			echo "failed for: Users6, fieldtypeid: $fieldTypeId";
			$success = false;
		}
		if (!$success) {
			echo "Migration Query Failed";
	...
	function addEntriesForSettings() {
		...
		$links = array(
			'index.php?module=Users6&parent=Settings&view=List',
