...
/*********************************************************************************
 * $Header: /advent/projects/wesat/vtiger_crm/sugarcrm/modules/Users6/Users6.php,v 1.10 2005/04/19 14:40:48 ray Exp $
...
require_once('modules/Users6/UserTimeZonesArray.php');
 ...
 *
 */
class Users6 extends CRMEntity {
    var $log;
    /**
     * @var PearDatabase
     */
    ...
    function Users6() {
        ...
        $this->log->debug("Entering Users6() method ...");
        $this->db = PearDatabase::getInstance();
        $this->DEFAULT_PASSWORD_CRYPT_TYPE = (version_compare(PHP_VERSION, '5.3.0') >= 0)?
                'PHP5.3MD5': 'MD5';
        $this->column_fields = getColumnFields('Users6');
        ...
        $this->log->debug("Exiting Users6() method ...");
    }
    ...
    function doLogin($user_password) {
        ...
        switch (strtoupper($AUTHCFG['authType'])) {
            case 'LDAP':
                ...
                require_once('modules/Users6/authTypes/LDAP.php');
            ...
            case 'AD':
                ...
                require_once('modules/Users6/authTypes/adLDAP.php');
                $adldap = new adLDAP();
...
//        $this->db->startTransaction();
    ...
    function fill_in_additional_detail_fields() {
        ...
        if($row != null) {
            $this->reports_to_name = stripslashes(getFullNameFromArray('Users6', $row));
        }
        else {
    ...
    function saveentity($module) {
        ...
        require_once('modules/Users6/CreateUserPrivilegeFile.php');
    ...
    function initSortByField($module) {
        // Right now, we do not have any fields to be handled for Sorting in Users6 module. This is just a place holder as it is called from Popup.php
    }
    ...
    function deleteImage() {
        ...
        if ($this->db->num_rows($res1) > 0) {
            ...
            $sql2 = "DELETE FROM vtiger_crmentity WHERE crmid=? AND setype='Users6 Attachments'";
     ...
     * @return Users6 - Active Admin User Instance
     */
