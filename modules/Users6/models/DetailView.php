...
class Users6_DetailView_Model extends Vtiger_DetailView_Model {
		...
		if (($currentUserModel->isAdminUser() == true || $currentUserModel->get('id') == $recordId) && $recordModel->get('status') == 'Active' ) {
			...
			$detailViewLinks = array(
				...
				array(
					...
					'linkurl' => "javascript:Users6_Detail_Js.triggerChangePassword('index.php?module=Users6&view=EditAjax&mode=changePassword&recordId=$recordId','Users6')",
					'linkicon' => ''
				)
			);
			...
			if($currentUserModel->isAdminUser() && $currentUserModel->get('id') != $recordId){
				$detailViewActionLinks = array(
					array(
						...
						'linkurl' => 'javascript:Users6_Detail_Js.triggerDeleteUser("' . $recordModel->getDeleteUrl() . '")',
						'linkicon' => ''
					)
				);
