...
class Users6_Record_Model extends Vtiger_Record_Model {
	...
	public function getProfileUrl() {
		$module = $this->getModule();
		return 'index.php?module=Users6&view=ChangePassword&mode=Profile';
	}
	 ...
	 * @return Users6_Record_Model instance
	 */
	...
	/**
	 * Static Function to get the instance of the User Record model from the given Users6 object
	 * @return Users6_Record_Model instance
	 */
	public static function getInstanceFromUserObject($userObject) {
		...
		return $userModel->setData($userObject->column_fields)->setModule('Users6')->setEntity($userObject);
	}

	/**
	 * Static Function to get the instance of all the User Record models
	 * @return <Array> - List of Users6_Record_Model instances
	 */
	public static function getAll($onlyActive=true) {
		...
		$noOfUsers6 = $db->num_rows($result);
		$users = array();
		if($noOfUsers6 > 0) {
			$focus = new Users6();
			for($i=0; $i<$noOfUsers6; ++$i) {
				$userId = $db->query_result($result, $i, 'id');
				$focus->id = $userId;
				$focus->retrieve_entity_info($userId, 'Users6');
	 ...
	 * @return <Array>
	 */
	function getSubordinateUsers6() {
		$privilegesModel = $this->get('privileges');

		if(empty($privilegesModel)) {
			$privilegesModel = Users6_Privileges_Model::getInstanceById($this->getId());
			$this->set('privileges', $privilegesModel);
		}

		$subordinateUsers6 = array();
		$subordinateRoleUsers6 = $privilegesModel->get('subordinate_roles_users');
		if($subordinateRoleUsers6) {
			foreach($subordinateRoleUsers6 as $role=>$users) {
				foreach($users as $user) {
					$subordinateUsers6[$user] = $privilegesModel->get('first_name').' '.$privilegesModel->get('last_name');
				}
			}
		}
		return $subordinateUsers6;
	}

	/**
	 * Function returns the Users6 Parent Role
	 * @return <String>
	 */
	...
	/**
	 * Function returns the Users6 Current Role
	 * @return <String>
	 */
	function getRole() {
		$privilegesModel = $this->get('privileges');
	...
	/**
	 * Function returns List of Accessible Users6 for a Module
	 * @param <String> $module
	 * @return <Array of Users6_Record_Model>
	 */
	public function getAccessibleUsers6ForModule($module) {
		$currentUser = Users6_Record_Model::getCurrentUserModel();
		$curentUserPrivileges = Users6_Privileges_Model::getCurrentUserPrivilegesModel();

		if($currentUser->isAdminUser() || $curentUserPrivileges->hasGlobalWritePermission()) {
			$users = $this->getAccessibleUsers6("",$module);
		} else {
			$sharingAccessModel = Settings_SharingAccess_Module_Model::getInstance($module);
			if($sharingAccessModel->isPrivate()) {
				$users = $this->getAccessibleUsers6('private',$module);
			} else {
				$users = $this->getAccessibleUsers6("",$module);
			}
		}
	 ...
	 * @return <Array>
	 */
	public function getAccessibleUsers6($private="",$module = false) {
        ...
        if(empty($accessibleUser)) {
			if($currentUserRoleModel->get('allowassignedrecordsto') === '1' || $private == 'Public') {
				...
				$accessibleUser = $this->getSameLevelUsers6WithSubordinates();
			} else if($currentUserRoleModel->get('allowassignedrecordsto') === '3') {
				$accessibleUser = $this->getRoleBasedSubordinateUsers6();
			}
	...
	/**
	 * Function to get same level and subordinates Users6
	 * @return <array> Users6
	 */
	public function getSameLevelUsers6WithSubordinates(){
		$currentUserRoleModel = Settings_Roles_Record_Model::getInstanceById($this->getRole());
		$sameLevelRoles = $currentUserRoleModel->getSameLevelRoles();
		$sameLevelUsers6 = $this->getAllUsers6OnRoles($sameLevelRoles);
		$subordinateUsers6 = $this->getRoleBasedSubordinateUsers6();
		foreach ($subordinateUsers6 as $userId => $userName) {
			$sameLevelUsers6[$userId] = $userName;
		}
		return $sameLevelUsers6;
	}
	
	/**
	 * Function to get subordinates Users6
	 * @return <array> Users6
	 */
	public function getRoleBasedSubordinateUsers6(){
		$currentUserRoleModel = Settings_Roles_Record_Model::getInstanceById($this->getRole());
		$childernRoles = $currentUserRoleModel->getAllChildren();
		$users = $this->getAllUsers6OnRoles($childernRoles);
	 ...
	 * @return <array>
	 */
	public function getAllUsers6OnRoles($roles) {
		$db = PearDatabase::getInstance();
		$roleIds = array();
		...
		if($noOfUsers6 > 0) {
			...
			for($j=0; $j<$noOfUsers6; ++$j) {
				...
				$subUsers6[$userId] = $firstName .' '.$lastName;
			}
		}
		return $subUsers6;
	}
    ...
    public static function getCount($onlyActive = false) {
        ...
        $numOfUsers6 = $db->num_rows($result);
        return $numOfUsers6;
    }
	...
	public static function getInstanceByName($userName) {
		...
		if ($db->num_rows($result)) {
			return Users6_Record_Model::getInstanceById($db->query_result($result, 0, 'id'), 'Users6');
		}
		return false;
	}
	...
	public function getActiveAdminUsers6() {
		$db = PearDatabase::getInstance();
