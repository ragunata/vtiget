...
require_once('modules/Users6/Users6.php');
require_once('include/utils/UserInfoUtil.php');
require_once('include/utils/utils.php');
require_once('include/utils/GetUserGroups.php');
require_once('include/utils/GetGroupUsers6.php');
...
function createUserPrivilegesfile($userid)
{
	...
	if($handle)
	{
		...
		$user_focus= new Users6();
		$user_focus->retrieve_entity_info($userid,"Users6");
		$userInfo=Array();
		...
		else
		{
			...
			$subRoleAndUsers6=getSubordinateRoleAndUsers6($user_role);
			...
			$newbuf .="\n";		
			$newbuf .= "\$subordinate_roles_users=".constructTwoDimensionalCharIntSingleArray($subRoleAndUsers6).";\n";
			$newbuf .="\n";		
			$newbuf .= "\$user_info=".constructSingleStringKeyValueArray($userInfo).";\n";
		...
		for($i=0;$i<$num_rows;$i++)
		{
			...
			if($share_permission == 1)
			{
				...
				{	
					...
					{
						$focusGrpUsers6 = new GetGroupUsers6();
						$focusGrpUsers6->getAllUsers6InGroup($share_grpid);
						$share_grp_users=$focusGrpUsers6->group_users;
						$share_grp_subgroups=$focusGrpUsers6->group_subgroups;
						$grp_read_per[$share_grpid]=$share_grp_users;
						foreach($focusGrpUsers6->group_subgroups as $subgrpid=>$subgrpusers)
						{
						...
						if($share_permission == 1)
						{
							if($def_org_share[$share_mod_id] == 3)
							{
								...
								if(! array_key_exists($shareEntId,$grp_read_per))
								{
									...
									{
										$focusGrpUsers6 = new GetGroupUsers6();
										$focusGrpUsers6->getAllUsers6InGroup($shareEntId);
										$share_grp_users=$focusGrpUsers6->group_users;
