...
class Users3_DetailView_Model extends Vtiger_DetailView_Model {
		...
		if (($currentUserModel->isAdminUser() == true || $currentUserModel->get('id') == $recordId) && $recordModel->get('status') == 'Active' ) {
			...
			$detailViewLinks = array(
				...
				array(
					...
					'linkurl' => "javascript:Users3_Detail_Js.triggerChangePassword('index.php?module=Users&view=EditAjax&mode=changePassword&recordId=$recordId','Users')",
					'linkicon' => ''
				)
			);
			...
			if($currentUserModel->isAdminUser() && $currentUserModel->get('id') != $recordId){
				$detailViewActionLinks = array(
					array(
						...
						'linkurl' => 'javascript:Users3_Detail_Js.triggerDeleteUser("' . $recordModel->getDeleteUrl() . '")',
						'linkicon' => ''
					)
				);
